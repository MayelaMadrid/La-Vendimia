...
app.use(cors());
const SELECT_CATALOGO_ARTICULOS =' SELECT * from articulos';
const SELECT_CATALOGO_CLIENTES = "SELECT * from clientes";
const SELECT_CATALOGO_VENTAS = "SELECT	v.folio, v.id_cliente , concat(c.nombre,  ' '  ,c.apellido_paterno, ' '  ,c.apellido_materno) as nombre , v.total, v.fecha FROM	ventas v	INNER JOIN clientes c ON v.id_cliente = c.id_cliente";
...
app.get('/clientes/actualizarCliente',(req,res) =>{
const { nombre, apellido_materno, apellido_paterno, rfc ,id_cliente} = req.query;
const ACTUALIZAR_CLIENTE =   `UPDATE clientes  SET  nombre = '${ nombre }', apellido_paterno = '${ apellido_paterno }', apellido_materno = '${ apellido_materno }', RFC = '${ rfc }' WHERE id_cliente = '${ id_cliente }'`;
